...
exports.run = async (client, message, args) => {
    ...
    const privateChannelID = '934442750802280478'; // ! Değişecek
    const categoryID = '934439232523612170'; // ! Değişecek
    ...
    try {
        switch (option) {
            case 'oluştur':
                ...
                if (isNaN(limit)) return error('Maksimum üye sayısını geçtiniz limit **99!**');
                if (limit >= 100) return error('Maksimum üye sayısını geçtiniz limit **99!**');
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['ök', 'özel-kanal','?','özel-oda'],
    permLevel: 0
};
